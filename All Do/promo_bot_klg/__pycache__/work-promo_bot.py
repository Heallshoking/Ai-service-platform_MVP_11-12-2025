...
try:
    ...
    logging.warning("⚠️ apscheduler not installed. Push notifications disabled. Install with: pip install apscheduler")
...
DEFAULT_SETTINGS = {
    ...
    "LOCATION_MAX_AGE_MINUTES": "7"  # 🔧 ИСПРАВЛЕНО: 15→7 минут
}
...
def init_sheets() -> None:
    ...
    try:
        ...
        try:
            ...
            finance_sheet = sheet.add_worksheet(title="Финансы", rows=500, cols=10)
            finance_sheet.update(values=[[
                "Дата", "Промоутер", "Адрес", "Район", "Тип", "Категория", "Количество", "Цена за единицу", "Сумма", "Комментарий"
            ]], range_name="A1:J1")
            logging.info("✅ Лист 'Финансы' создан")
        ...
        # ИСПРАВЛЕНО: Проверяем и создаём колонки в таблице "Балансы" (5 колонок: Промоутер, Баланс, Заметки, Телефон, Имя)
        try:
            headers = balances_sheet.row_values(1)
            if not headers:
                # Если заголовков нет, создаём их
                balances_sheet.update(values=[["Промоутер", "Баланс", "Заметки", "Телефон", "Имя"]], range_name="A1:E1")
                logging.info("✅ Созданы заголовки в таблице 'Балансы'")
            else:
                # Добавляем недостающие колонки
                if "Телефон" not in headers:
                    if len(headers) < 4:
                        while len(headers) < 3:
                            headers.append("")
                        ...
                        headers[3] = "Телефон"
                    logging.info("✅ Добавлена колонка 'Телефон' в таблицу 'Балансы'")

                if "Имя" not in headers:
                    if len(headers) < 5:
                        while len(headers) < 4:
                            headers.append("")
                        headers.append("Имя")
                        ...
                        headers[4] = "Имя"
                    logging.info("✅ Добавлена колонка 'Имя' в таблицу 'Балансы'")

                # Обновляем заголовки (новый порядок аргументов: values, range_name)
                balances_sheet.update(values=[headers[:5]], range_name="A1:E1")
        except Exception as e:
    ...
    try:
        ...
        for address in raw_addresses:
            ...
            if normalized_addr in existing_addresses:
                ...
                # Добавляем новый
                priority_addresses_sheet.append_row([address, current_time, status, "100", last_visit])
                added_count += 1
...
def get_priority_addresses(force: bool = False) -> List[Dict[str, Any]]:
    """
...
def get_or_create_nearby_addresses(
    ...
    try:
            ...
            for dist_m in distances:
                ...
                for angle in angles:
                    ...
                    try:
                        ...
                        logging.info(f"✅ Добавлен новый адрес в Справочник: {new_addr} ({district}, {score} pts)")
                    except Exception as e:
        ...
        priorities = load_address_priorities()
        
        if priorities:
            # Сортируем: сначала по приоритету (больше = лучше), потом по расстоянию (меньше = ближе)
            candidates.sort(key=lambda x: (-priorities.get(x[1], 50), x[0]))
            logging.info(f"🎯 Адреса отсортированы по приоритету")
        else:
            # Фолбэк: если приоритеты не загрузились, сортируем только по расстоянию
...
def register_user(user_id: int, phone: str, name: str) -> bool:
    """Регистрирует нового пользователя в Google Sheets 'Балансы'"""
    try:
        if not balances_sheet:
        ...
        # Добавляем новую строку с 10 колонками (как в daily summary)
        # [ПромоутерID, Дата, Тип, Листовки (шт), Фото двери (шт), Фото щитов (шт), Оплата дверь (₽), Оплата щиты базовая (₽), Премия активность (₽), Итого (₽)]
        new_row = [
            str(user_id),       # ПромоутерID
            datetime.now().strftime("%d.%m.%Y"),  # Дата регистрации
            "Регистрация",      # Тип
            "0",               # Листовки (шт)
            "0",               # Фото двери (шт)
            "0",               # Фото щитов (шт)
            "0.00",            # Оплата дверь (₽)
            "0.00",            # Оплата щиты базовая (₽)
            "0.00",            # Премия активность (₽)
            "0.00"             # Итого (₽)
        ]
        balances_sheet.append_row(new_row)
        logging.info(f"✅ Пользователь зарегистрирован: ID={user_id}, Телефон={phone}, Имя={name}")
        return True
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # 🛡️ СБРОС СОСТОЯНИЯ: пользователь вернулся в меню
        ...
        if is_user_registered(user_id):
            ...
            welcome_text = (
                f"👋 Привет, {user_name}!\n\n"
                f"📍 Я — бот для промоутеров Калининграда.\n"
                f"📊 Помогу тебе управлять адресами, фотоотчётами и заработком!\n\n"
...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    profile_text = (
        ...
        f"└─ 🚪 Ставка у домофона: {effective_door_rate:.1f}₽/фото\n"
        f"└─ ⚡ Ставка электрощита: {effective_panel_rate:.1f}₽/фото\n\n"
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if selected_address:
            ...
            if distance and distance > LOCATION_RADIUS_METERS:
                await update.message.reply_text(
                    ...
                    f"🎯 Рекомендуем подойти ближе (в пределах {LOCATION_RADIUS_METERS} м).\n\n"
                    f"💡 Но если ты уверен — продолжай!\n\n"
        ...
        result_text += "\n".join(address_list)
        result_text += "\n\n💬 Напиши свой адрес, Ян обновит карту!"
        ...
        # 🔧 ИСПРАВЛЕНО: Отправляем 2 сообщения для надёжного отображения клавиатуры
        # Сообщение 1: Результаты сканирования (без кнопок)
        await update.message.reply_text(
            result_text,
            parse_mode="Markdown"
        )
        
        # Сообщение 2: Клавиатура с адресами
        await update.message.reply_text(
            "👇 Выбери адрес или напиши свой:",
            reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False)
        )
...
def get_flyer_balance(user_id: int) -> int:
    """Получает баланс листовок промоутера из листа 'Листовки'"""
    try:
        if not flyers_sheet:
            return 0
...
def create_flyer_request(user_id: int, user_name: str, quantity: int = 1000) -> bool:
    ...
    try:
        ...
        flyer_requests_sheet.append_row(new_row)
...
async def auto_update_statuses() -> None:
    ...
    try:
        ...
        for i, row in enumerate(all_values[1:], start=2):
            ...
            try:
                # Парсим дату последнего визита (DD.MM.YYYY HH:MM)
                last_visit_date = datetime.strptime(last_visit, "%d.%m.%Y %H:%M")
...
async def send_morning_reminder(application) -> None:
    ...
    # Проверка на дубликаты (защита от многократного запуска)
    with notification_lock:
        now = datetime.now()
        last_sent = last_notification_sent.get("morning")
        
        # Если сообщение отправлено в последние 5 минут - пропускаем
        if last_sent and (now - last_sent).total_seconds() < 300:  # 5 минут
            logging.warning(f"⚠️ Утреннее напоминание уже отправлено {int((now - last_sent).total_seconds())} сек. назад. Пропускаем.")
            return
        
        # Отмечаем время отправки
        last_notification_sent["morning"] = now
...
async def send_evening_reminder(application) -> None:
    ...
    with notification_lock:
        now = datetime.now()
        last_sent = last_notification_sent.get("evening")
        
        if last_sent and (now - last_sent).total_seconds() < 300:  # 5 минут
            logging.warning(f"⚠️ Вечернее напоминание уже отправлено {int((now - last_sent).total_seconds())} сек. назад. Пропускаем.")
            return
        
        last_notification_sent["evening"] = now
                    ...
                    if next_tier:
                        ...
                        if progress_percent >= 50:
                            ...
                            message = (
                                ...
                                f"⚡ Ставка электрощита: {effective_flyer_rate:.1f}₽/фото"
                            )
...
async def send_cleanup_warning(application) -> None:
    ...
    with notification_lock:
        now = datetime.now()
        last_sent = last_notification_sent.get("cleanup")
        
        if last_sent and (now - last_sent).total_seconds() < 300:  # 5 минут
            logging.warning(f"⚠️ Предупреждение об очистке уже отправлено {int((now - last_sent).total_seconds())} сек. назад. Пропускаем.")
            return
        
        last_notification_sent["cleanup"] = now
        ...
        for row in all_values[1:]:
            if len(row) > 0 and row[0]:
                try:
                    ...
                    # Не отслеживаем приветственное сообщение нового дня - оно остаётся стартовым
                    await application.bot.send_message(
                        chat_id=uid,
                        text=(
...
def get_photo_price() -> float:
    ...
    try:
        ...
        load_settings()
        price_str = SETTINGS.get("Стоимость за 1 фото", "3.0")  # 🔧 ИСПРАВЛЕНО: 10→3.0₽ (базовая ставка по прайсу)
        return float(price_str)
    except Exception as e:
        logging.warning(f"⚠️ Ошибка получения цены фото: {e}")
        return 3.0  # 🔧 ИСПРАВЛЕНО: Цена по умолчанию 3.0₽
...
def deduct_flyers(user_id: int, count: int) -> bool:
    """
    🔥 УЛУЧШЕНО: Списывает листовки + начисляет деньги за листовки!
    Система: 5₽/шт + бонусы за streak (3 дня +20%, 5 дней +50%)
    """
    try:
        if not flyers_sheet:
            logging.error("❌ flyers_sheet не инициализирован")
            return False
        
        all_values = flyers_sheet.get_all_values()
        if len(all_values) <= 1:
            logging.error("❌ Лист 'Листовки' пуст")
            return False
        
        for i, row in enumerate(all_values[1:], start=2):
            if len(row) > 0 and str(row[0]) == str(user_id):
                # Структура: [Промоутер, Баланс]
                current_balance = 0
                ...
                flyers_sheet.update_cell(i, 2, str(new_balance)
                )
        ...
        logging.warning(f"⚠️ Промоутер {user_id} не найден в 'Листовки'")
        return False
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if state == "awaiting_door_photo":
            ...
            load_settings()
            loc_max_age = int(SETTINGS.get("LOCATION_MAX_AGE_MINUTES", "15"))
            ...
            # Обновляем статус
            update_address_status(selected_address, "🟡 Нет доступа")
            ...
            if "current_location" in user_state[user_id]:
                ...
                if nearby_addresses:
                    ...
                    result_text = (
                        ...
                        f"🟡 Адрес отмечен как 'Нет доступа'.\n"
                    ...
                    await update.message.reply_text(
                        f"✅ **Фото двери принято!**\n\n"
                        f"🟡 Адрес {selected_address} отмечен как 'Нет доступа'.\n"
        ...
        # 📸 🚪 Фото двери при ВЫХОДЕ (ОБЯЗАТЕЛЬНОЕ)
            ...
            update_address_status(selected_address, "🟢 Показы идут", photos_count=photos_uploaded)
            ...
            await update.message.reply_text(
                ...
                f"📄 <b>Сколько электрощитов с нАШИМИ листовками уже было в доме?</b>\n"
                f"(до твоей работы)\n\n"
                f"🔢 Выбери количество:",
                parse_mode="HTML",
            ...
            if "current_location" in user_state[user_id]:
                ...
                if nearby_addresses:
                    ...
                    await update.message.reply_text(
                        ...
                        f"📡 Отправь геолокацию для сканирования района или добавь новый адрес!",
                        parse_mode="Markdown",
                        reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
                    )
            else:
        ...
        # 📸 Фото электрощитов
            ...
            try:
                ...
                logging.info(f"✅ Листовки списаны: {user_id} -1 шт (по {effective_panel_rate:.1f}₽/фото)")
            ...
            # Обновляем сессионный счётчик
            update_session_stats(user_id, photos=1, earnings=photo_price)
            stats = get_session_stats(user_id)
            total_amount = stats['earnings']  # 🔥 ПОЛУЧАЕМ total_amount из stats
    ...
    try:
        ...
        if current_state in working_states:
            if text in {"Начать работу 🚀", "📍 Добавить адрес"} or looks_like_address(text):
                current_address = user_state.get(user_id, {}).get("current_address", "текущий адрес")
                ...
                if current_state == "awaiting_photos":
                    ...
                    msg = (
                        "🚫 Сейчас идёт работа по адресу: " + str(current_address) + "\n\n"
                        "📸 Заверши фотоотчёт, затем можно сменить адрес."
                    )
                elif current_state == "awaiting_access_answer":
                    keyboard = ReplyKeyboardMarkup([["✅ Да!", "🚪 Нет доступа"],["📍 Добавить адрес"]], resize_keyboard=True, one_time_keyboard=False)
                    msg = (
                        "🚫 Сейчас идёт работа по адресу: " + str(current_address) + "\n\n"
                        "🔑 Подтверди доступ по текущему адресу, затем можно сменить адрес."
                    )
                else:
        ...
        # ✅ Завершить работу (оплата + следующие адреса)
            ...
            if photos_uploaded < min_photos:
                await update.message.reply_photo(
                    photo="https://disk.yandex.ru/i/xWAtwVqcN7H9zQ",
                    caption=(
                        f"✅ Этап можно завершить с {photos_uploaded} фото.\n\n"
                        f"💡 Рекомендация: сделай минимум {min_photos} фото для большего заработка.\n"
                        f"📈 Внутренние щиты оплачиваются выше.\n\n"
                        f"🚪 Теперь обязательно сфотографируй входную дверь с визиткой."
                    ),
                    reply_markup=ReplyKeyboardMarkup([["❌ Отмена"]], resize_keyboard=True)
                )
            
            # Получаем инфо об адресе
            selected_address = user_state[user_id].get("selected_address", "Неизвестный адрес")
            addr_info = user_state[user_id].get("address_info")
            # ЗАЩИТА: Проверяем, что addr_info существует и имеет правильную длину
            district = addr_info[3] if (addr_info and len(addr_info) > 3) else "Неизвестный"
            
            # Рассчитываем оплату (для отображения)
            photo_price = get_photo_price()
            total_amount = photos_uploaded * photo_price
            
            # ВАЖНО: Оплата и листовки уже обновлены при каждом фото!
            # Теперь запрашиваем ОБЯЗАТЕЛЬНОЕ фото двери с визиткой!
            ...
            # Убираем клавиатуру и просим фото двери
            await update.message.reply_photo(
                photo="https://disk.yandex.ru/i/xWAtwVqcN7H9zQ",
                caption=(
        ...
        # Вернуться в меню или Отмена
        elif text == "Вернуться в меню" or text == "❌ Отмена":
        ...
        # ✅ 📍 Я на месте! (подтверждение местоположения)
            ...
            # Спрашиваем про доступ в подъезд (кнопки на одной линии)
            keyboard = [
                ["✅ Да!", "🚪 Нет доступа"]
            ]
            ...
            # 🔥 Формируем мотивационное сообщение о последнем посещении
            if last_visit and last_visit != "—":
                last_visit_text = f"🕒 Последнее посещение: {last_visit}"
            else:
                last_visit_text = ""
            
            # 🔥 Отправляем фото с простым вопросом
            try:
                from telegram.error import TelegramError, BadRequest
                # Формируем подпись с учётом бонуса для только что добавленного адреса
                if user_state.get(user_id, {}).get("just_added_address"):
                    caption_text = f"🎉 Ты здесь первопроходец! +10% бонус\n\n🎯 Есть доступ в подъезд?"
                    # Сбрасываем флаг после показа
                    user_state[user_id]["just_added_address"] = False
                else:
                    if last_visit and last_visit != "—":
                        caption_text = f"🎯 Отлично! Есть доступ в подъезд?\n🕒 Последнее посещение: {last_visit}"
                    else:
                        caption_text = "🎯 Отлично! Есть доступ в подъезд?"
                await update.message.reply_photo(
                    photo="https://disk.yandex.ru/i/5IsEqKDk2lopxg",  # 🔥 Фото промоутера на месте
        ...
        # ✅ Да! (есть доступ в подъезд)
            ...
            await update.message.reply_photo(
                ...
                caption=(
                    ...
                    f"📍 <b>{address}</b>\n\n"
                    f"Инструкция:\n"
                    f"📸 Зафиксировать визитку на электрощит\n"
                    f"📤 Добавить фотографию в чат\n\n"
                    f"ℹ️ Рекомендация: начинай с последнего этажа."
                ),
                parse_mode="HTML",
                reply_markup=ReplyKeyboardMarkup([["📤 Добавить несколько фото"],["💾 Сохранить"]], resize_keyboard=True)  # 🔧 ИСПРАВЛЕНО: скрыта кнопка "Зафиксировать листовку"
            )
        ...
        # 📤 Режим добавления нескольких фото
            ...
            if user_state.get(user_id, {}).get("state") == "awaiting_photos":
                await update.message.reply_text(
                    ...
                    reply_markup=ReplyKeyboardMarkup([["💾 Сохранить"]], resize_keyboard=True)
                )
            else:
        ...
        # 📸 Зафиксировать листовку (подсказка для промоутера)
            ...
            await update.message.reply_text("📡 Добавляю адрес…\n🔄 Проверяю район и координаты…")
            result = geocode_address(manual_address)
            if not result:
                await update.message.reply_text(
                    f"❌ Адрес \"{manual_address}\" не найден на карте.\n\n"
                    f"📌 Проверь правильность написания (например: <i>Елизаветинская 5</i>).",
                    parse_mode="HTML",
                    reply_markup=get_main_menu_keyboard()
                )
                user_state[user_id]["state"] = None
            ...
            if not addr_info:
                try:
                    ...
                    logging.info(f"✅ Адрес '{manual_address}' добавлен: ({addr_lat}, {addr_lng}, {addr_district})")
        ...
        # Обработка ввода адреса
            ...
            if user_id in user_state and user_state[user_id].get("state") == "awaiting_manual_address":
                ...
                if not result:
                    await update.message.reply_text(
                        f"❌ Адрес \"{manual_address}\" не найден на карте.\n\n"
                        f"📌 Проверь правильность написания (например: <i>Еловая 40</i>).",
                        parse_mode="HTML",
                        reply_markup=get_main_menu_keyboard()
                    )
                    user_state[user_id]["state"] = None
                ...
                addr_lat, addr_lng, addr_district = result  # 🔥 НОВОЕ: Получаем район автоматически!
                # Корректируем район по координатам и подсказкам улиц
                addr_district = ensure_real_district(manual_address, addr_lat, addr_lng, addr_district)
                
                # Проверяем, есть ли адрес в справочнике
                addr_info = get_address_info(manual_address)
                if not addr_info:
                    # 🔥 ИСПРАВЛЕНО: Добавляем координаты в ПРАВИЛЬНЫЕ столбцы (H=8, I=9)
                    # 🗺️ НОВОЕ: Используем РЕАЛЬНЫЙ район из геокодера!
                    try:
                        sprav.append_row([
                            ...
                            str(addr_lat),        # H: Широта (lat)
                            str(addr_lng)         # I: Долгота (lng)
                        ])
                        logging.info(f"✅ Адрес '{manual_address}' добавлен: ({addr_lat}, {addr_lng}, {addr_district})")
                ...
                if addr_info:
                    ...
                    if len(addr_info) != 7:
                        ...
                        user_state[user_id]["state"] = None
                        return
                    # ИСПРАВЛЕНО: Распаковываем все 7 значений (не [:5])
                    addr, lat, lng, district, status_card, last_promoter, last_visit = addr_info
                ...
                # ВАЖНО: Очищаем текст от эмодзи статусов и расстояния (например, "🟡 Еловая 48 (18 м)" → "Еловая 48")
                clean_text = text
                ...
                if not looks_like_address(clean_text):
                    await update.message.reply_text(
                ...
                if addr_info:
                    ...
                    # 🔄 Отправляем милое уведомление о подготовке
                    loading_messages = [
                        "🎯 Отличный выбор! Готовим адрес для работы...",
                        "🚀 Загружаем данные... Скоро начнём!",
                        "⚡ Адрес обрабатывается... Оставайся на связи!",
                        "🎨 Формируем маршрут... Почти готово! ~30 сек",
                        "✨ Подготавливаем информацию... Момент!",
                        "🔥 Адрес в работе... Секундочку!",
                        "💫 Загружаем детали... ~30 сек",
                    ]
                    import random
                    await update.message.reply_text(random.choice(loading_messages))
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if state == "awaiting_photos":
            ...
            keyboard = ReplyKeyboardMarkup([["✅ Да!", "🚪 Нет доступа"],["📍 Добавить адрес"],["🆘 Сообщить о проблеме"]], resize_keyboard=True, one_time_keyboard=False)
            message = "❌ Произошла ошибка.\n\n🔑 Подтверди доступ: выбери один из вариантов.\n\n💡 Если проблема повторяется — нажми '🆘 Сообщить о проблеме'"
            ...
            keyboard = ReplyKeyboardMarkup([["🔍 Сканировать район"],["📍 Добавить адрес"],["🆘 Сообщить о проблеме"]], resize_keyboard=True, one_time_keyboard=False)
    ...
    try:
        ...
        for i, row in enumerate(all_values[1:], start=2):
            if len(row) >= 5 and str(row[0]) == str(promoter_id) and row[4] == "⏳ Ожидает":
                ...
                # Обновляем баланс в листе "Листовки"
                flyers_values = flyers_sheet.get_all_values()
                user_found = False
                
                for j, flyer_row in enumerate(flyers_values[1:], start=2):
                    if str(flyer_row[0]) == str(promoter_id):
                        flyers_sheet.update_cell(j, 2, str(new_balance))
                        user_found = True
                        break
                
                # Если пользователя нет в "Листовки" - добавляем
                if not user_found:
                    flyers_sheet.append_row([str(promoter_id), str(quantity)])
    ...
    try:
        ...
        try:
            ...
            if _pending_admin_notification:
                adm_kb = [
                    [InlineKeyboardButton("📦 500 шт", callback_data=f"approve_{user_id}_500"), InlineKeyboardButton("📦 1000 шт", callback_data=f"approve_{user_id}_1000")],
                    [InlineKeyboardButton("📦 1500 шт", callback_data=f"approve_{user_id}_1500"), InlineKeyboardButton("💯 Своё кол-во", callback_data=f"approve_{user_id}_custom")],
                ...
                admin_message = (
                    ...
                    f"📦 Количество: {_pending_admin_notification['quantity']} листовок\n"
                    f"⏰ Дата: {_pending_admin_notification['request_date']}\n\n"
                    f"⚡ Выбери количество листовок для одобрения:"
                )
                for admin_id in ADMIN_IDS:
                    try:
...
async def handle_admin_approve_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Обрабатывает: approve_{user_id}_{quantity}, approve_{user_id}_custom, reject_{user_id}
    """
    try:
        query = update.callback_query
        ...
        if query.data.startswith("approve_"):
            ...
            if quantity_param == "custom":
                # Создаём калькулятор для ввода количества
                # InlineKeyboardButton, InlineKeyboardMarkup импортируются на уровне модуля
                
                # Калькулятор: цифры + контрольные кнопки
                keyboard = [
            ...
            if success:
                # 🔥 ОТКРЕПЛЯЕМ сообщение после одобрения
                try:
                    ...
                    # Открепляем закрепы у других админов и у промоутера
                    for other_admin in ADMIN_IDS:
                        key = (other_admin, promoter_id)
                        msg_id = pinned_admin_request_messages.get(key)
                        if msg_id:
                            try:
                                await context.bot.unpin_chat_message(chat_id=other_admin, message_id=msg_id)
                            except Exception:
                                pass
                            finally:
                                del pinned_admin_request_messages[key]
                    pm_msg_id = pinned_promoter_request_messages.get(promoter_id)
                    if pm_msg_id:
                        try:
        ...
        # ❌ Отклонение заявки
            ...
            promoter_id = int(query.data.split("_")[1])
            
            # 🔥 ОТКРЕПЛЯЕМ сообщение при отклонении
            try:
                await context.bot.unpin_chat_message(
                    chat_id=admin_id,
                    message_id=query.message.message_id
                )
                logging.info(f"📍 Сообщение с заявкой откреплено после отклонения")
                # Открепляем закрепы у других админов и у промоутера
                for other_admin in ADMIN_IDS:
                    key = (other_admin, promoter_id)
                    msg_id = pinned_admin_request_messages.get(key)
            ...
            for i, row in enumerate(all_values[1:], start=2):
                if len(row) >= 5 and str(row[0]) == str(promoter_id) and row[4] == "⏳ Ожидает":
                    flyer_requests_sheet.update_cell(i, 5, "❌ Отклонено")
                    logging.info(f"❌ Админ {admin_id} отклонил заявку: {promoter_id}")
                    break
...
async def handle_promoter_calculator_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if data.startswith("req_cancel_custom_"):
            await query.edit_message_text("❌ Отмена ввода количества")
            return
        ...
        for i, row in enumerate(all_values[1:], start=2):
            if len(row) >= 5 and str(row[0]) == str(promoter_id) and row[4] == "⏳ Ожидает":
                ...
                try:
                    ...
                    activity_multiplier = min(1.0 + 0.10 * streak_days, 1.5)
                    bonus_text = "🔥 Активность: +{}%".format(int((activity_multiplier - 1.0)*100))
                    effective_panel_rate = 3.0 * activity_multiplier * user_state.get(promoter_id, {}).get("address_bonus_multiplier", 1.0)
                    
                    # Рассчитываем потенциальный заработок
                    potential_earnings = calculate_flyer_earnings(promoter_id, quantity)
                    ...
                    await context.bot.send_message(
                        ...
                        text=(
                            ...
                            f"📦 Получено: **{quantity} листовок**\n"
                            f"💼 Новый баланс: **{new_balance} шт**\n\n"
                            f"💰 Потенциальный заработок: **{potential_earnings:.2f}₽** (по {effective_panel_rate:.1f}₽/фото)\n"
                            f"🔥 Активность: **{streak_days} дн.** ({bonus_text})\n\n"
                            f"🚀 Теперь можешь начать работу!\n"
                            f"✅ Нажми '🚀 Начать работу'"
                        ),
...
def main() -> None:
    ...
    try:
        ...
        application.add_handler(CallbackQueryHandler(handle_admin_approve_callback, pattern="^(approve_|reject_)"))
