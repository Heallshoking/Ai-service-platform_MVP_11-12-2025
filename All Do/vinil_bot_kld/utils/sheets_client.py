...
class SheetsClient:
    ...
    def add_record(self, record_data: Dict) -> int:
        ...
        try:
            ...
            row = [
                ...
                record_data.get('status', '🟢 Доступна'),  # Статус (можно передать)
                '',                                       # Описание
                '',                                       # SEO_Title
                '',                                       # SEO_Description
    ...
    def update_description(self, row_number: int, description: str):
        ...
        try:
            worksheet = self.spreadsheet.worksheet(self.SHEET_CATALOG)
            # Колонка 14 - Описание (схема: 13й элемент массива)
            worksheet.update_cell(row_number, 14, description)
    ...
    def update_status(self, row_number: int, status: str):
        ...
        Args:
            row_number: Номер строки
            status: Новый статус (🟢 Доступна / 🟡 Предзаказ / 🔴 Продана)
        """
        try:
            worksheet = self.spreadsheet.worksheet(self.SHEET_CATALOG)
            # Колонка 13 - Статус (схема: 12й элемент массива)
            worksheet.update_cell(row_number, 13, status)
    ...
    def find_records_by_query(self, query: str, limit: int = 10) -> List[Dict]:
        """
        Поиск записей по названию, исполнителю или жанру (нечёткий поиск)
        ...
        try:
            ...
            results = []
            for idx, rec in enumerate(records):
                # Поиск в названии, исполнителе, жанре, артикуле
                title = str(rec.get('Название', '')).lower()
                artist = str(rec.get('Исполнитель', '')).lower()
                genre = str(rec.get('Жанр', '')).lower()
                article = str(rec.get('Артикул', '')).lower()
                
                # Проверяем совпадение
                if q in title or q in artist or q in genre or q in article:
                    item = dict(rec)
                    item['_row_number'] = idx + 2
                    results.append(item)
            ...
            logger.error(f"Ошибка поиска по запросу: {e}")
            return []
